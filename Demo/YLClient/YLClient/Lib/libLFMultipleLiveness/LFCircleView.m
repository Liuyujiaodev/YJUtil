...
- (void)drawWithAnglePercent
{
    ...
    CGContextAddArc(self.context , self.bounds.size.width / 2 ,
                    self.bounds.size.height / 2 ,
                    self.bounds.size.width / 2 - _fBodyWidth / 2,
                    -M_PI_2 ,
                    M_PI_2 * 3,
                    0);
    CGContextSetLineWidth(self.context, _fBodyWidth);
    CGContextSetStrokeColorWithColor(self.context, _colorB.CGColor);
    CGContextStrokePath(self.context);
    
    
    CGContextSetBlendMode(self.context, kCGBlendModeClear);
    
    CGContextAddArc(self.context ,
    ...
    CGContextSetLineWidth(self.context, _fBodyWidth + 1);
    CGContextStrokePath(self.context);
}
