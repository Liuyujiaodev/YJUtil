...
- (instancetype)init
{
    return [self initWithRepeatMaxCount:INFINITY withCompletionBlock:nil];
}

- (instancetype)initWithRepeatMaxCount:(NSInteger)repeatMaxCount
{
    return [self initWithRepeatMaxCount:repeatMaxCount withCompletionBlock:nil];
}

- (instancetype)initWithRepeatMaxCount:(NSInteger)repeatMaxCount withCompletionBlock:(YFGIFImageViewCompletionBlock)completionBlock
{
    self = [super init];
    if(self) {
        _gifPixelSize = CGSizeZero;
        _repeatMaxCount = repeatMaxCount;
...
- (void)play
{
    ...
    if (_index >= _frameCount) {
        _currentRepeatCount++;
        if (_currentRepeatCount >= self.repeatMaxCount) {
            [self stopGIF];
...
{
    return _startTime ? [[NSDate date] timeIntervalSinceDate:_startTime] : 0;
}
